...
import { SVGLoader } from '../../twod/SVGLoader.js';  // a patched version to handle text / clipmaps
...
H.renderer.three = class {
  ...
  */
  constructor(element, cfg, onStateChange) {
    ...
    element.addEventListener('mousedown', (this.onClick).bind(this), false);
  }

  initThree() {

    // if (this.eCfg.renderObj) {
    //   this.camera.
    // } else {
      this.camera = new THREE.PerspectiveCamera( 50, this.eCfg.sizing.width/this.eCfg.sizing.height, 1, 1000 );
  ...
  render(svg, events) {
    ...
    var loader = new SVGLoader();
      ...
      for ( var i = 0; i < paths.length; i ++ ) {
        ...
        if (fillColor == 'none'){
          fillColor = '#000'; // some colors are "none" which is invalid HTML
          opacity = 0;
        }
        ...
        if (needsGradient){
          ...
          let dasharray = ((path.userData.node.getAttribute('stroke-dasharray'))||'1 0').split(' ').map(d => parseFloat(d));
          if (dasharray.length == 1) dasharray.push(dasharray[0]);
          if (dasharray.length > 2) console.warn('THREE parses only the first two elements of dasharray.');
      ...
      if (this.onStateChange)
        this.onStateChange('PARSE_THREE_SVG_END', group)
  ...
  genText(nodes, font) {
    ...
    loader.load( `fonts/${font}.ttf`, (function ( json ) {
      ...
      nodes.forEach((item, i) => {
        ...
        let text = item.userData.node.innerHTML;
  ...
  */
  attachEvents(group, events) {
    ...
    events.forEach((e, i) => {
      let item = group.children.filter(d=>d.name==e.elId);
      ...
      evTypes.forEach((evt, i) => {
        ...
        let uData = item[0].userData;
        if(!uData.event) uData.event = {};
        uData.event[eventName] = () => {         // init handler
            if (e.ev.group)
              H.Emitter.PubSub.publish(e.ev.group, {type:evt, e:e});
            if (e.ev[evt].element)
              this.on(item[0], evt, e.ev[evt].element); // paperjs .getFillColor does not work, so provide it.
            if (e.ev[evt].handler)
